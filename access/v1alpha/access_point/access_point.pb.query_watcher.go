...
type QueryWatcherParams struct {
	...
	Cursor       *access_point.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*access_point.AccessPointChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return access_point.AccessPointChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client access_point_client.AccessPointServiceClient,
...
func NewQueryWatcherWithIChan(id int, client access_point_client.AccessPointServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "accessPoint-query-watcher").
	...
	for {
		stream, err := qw.client.WatchAccessPoints(ctx, &access_point_client.WatchAccessPointsRequest{
		...
		if err != nil {
			...
			pending := make([]*access_point.AccessPointChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAccessPointChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetAccessPointChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(access_point.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*access_point.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*access_point.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*access_point.Filter)
		}
		return NewQueryWatcherWithIChan(id, access_point_client.NewAccessPointServiceClient(cc), cfg, ch)
	})
}
