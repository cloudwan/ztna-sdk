...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *access_point.AccessPoint_FieldMask
	OrderBy   *access_point.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *access_point.ParentReference
	Filter *access_point.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[access_point.Name]*access_point.AccessPoint
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*access_point.AccessPointChange
}

func NewWatcher(client access_point_client.AccessPointServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &access_point.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &access_point.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "accessPoint-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[access_point.Name]*access_point.AccessPoint{}
...
type WatcherEventChange struct {
	pre, post *access_point.AccessPoint
	name      *access_point.Name
}

func NewAddWatcherEventChange(resource *access_point.AccessPoint) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *access_point.AccessPoint) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *access_point.AccessPoint) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *access_point.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *access_point.AccessPoint {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *access_point.AccessPoint {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *access_point.AccessPoint {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *access_point.AccessPoint {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[access_point.Name]*access_point.AccessPoint, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		accessPoint := currentChange.GetCurrent()
		byName[*accessPoint.GetName()] = accessPoint
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			accessPoint := change.GetCurrent()
			byName[*accessPoint.GetName()] = accessPoint
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, accessPoint := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(accessPoint))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[access_point.Name]*access_point.AccessPoint{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *access_point.AccessPoint
		switch item := change.ChangeType.(type) {
		case *access_point.AccessPointChange_Added_:
			currentItem = item.Added.GetAccessPoint()
		case *access_point.AccessPointChange_Current_:
			currentItem = item.Current.GetAccessPoint()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*access_point.AccessPointChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *access_point.AccessPointChange_Added_:
			newItem := item.Added.GetAccessPoint()
		...
		case *access_point.AccessPointChange_Modified_:
			updatedItem := item.Modified.GetAccessPoint()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*access_point.AccessPointChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetAccessPointName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(access_point.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*access_point.AccessPoint_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*access_point.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(access_point_client.NewAccessPointServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(access_point.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*access_point.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*access_point.ParentReference)
		}
		return params
	})
}
