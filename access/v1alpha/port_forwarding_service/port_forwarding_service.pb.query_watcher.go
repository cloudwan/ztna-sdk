...
type QueryWatcherParams struct {
	...
	Cursor       *port_forwarding_service.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*port_forwarding_service.PortForwardingServiceChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return port_forwarding_service.PortForwardingServiceChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client port_forwarding_service_client.PortForwardingServiceServiceClient,
...
func NewQueryWatcherWithIChan(id int, client port_forwarding_service_client.PortForwardingServiceServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "portForwardingService-query-watcher").
	...
	for {
		stream, err := qw.client.WatchPortForwardingServices(ctx, &port_forwarding_service_client.WatchPortForwardingServicesRequest{
		...
		if err != nil {
			...
			pending := make([]*port_forwarding_service.PortForwardingServiceChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPortForwardingServiceChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPortForwardingServiceChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(port_forwarding_service.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*port_forwarding_service.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*port_forwarding_service.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*port_forwarding_service.Filter)
		}
		return NewQueryWatcherWithIChan(id, port_forwarding_service_client.NewPortForwardingServiceServiceClient(cc), cfg, ch)
	})
}
