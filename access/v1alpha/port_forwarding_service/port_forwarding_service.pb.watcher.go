...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *port_forwarding_service.PortForwardingService_FieldMask
	OrderBy   *port_forwarding_service.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *port_forwarding_service.ParentReference
	Filter *port_forwarding_service.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[port_forwarding_service.Name]*port_forwarding_service.PortForwardingService
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*port_forwarding_service.PortForwardingServiceChange
}

func NewWatcher(client port_forwarding_service_client.PortForwardingServiceServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &port_forwarding_service.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &port_forwarding_service.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "portForwardingService-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[port_forwarding_service.Name]*port_forwarding_service.PortForwardingService{}
...
type WatcherEventChange struct {
	pre, post *port_forwarding_service.PortForwardingService
	name      *port_forwarding_service.Name
}

func NewAddWatcherEventChange(resource *port_forwarding_service.PortForwardingService) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *port_forwarding_service.PortForwardingService) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *port_forwarding_service.PortForwardingService) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *port_forwarding_service.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *port_forwarding_service.PortForwardingService {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *port_forwarding_service.PortForwardingService {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *port_forwarding_service.PortForwardingService {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *port_forwarding_service.PortForwardingService {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[port_forwarding_service.Name]*port_forwarding_service.PortForwardingService, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		portForwardingService := currentChange.GetCurrent()
		byName[*portForwardingService.GetName()] = portForwardingService
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			portForwardingService := change.GetCurrent()
			byName[*portForwardingService.GetName()] = portForwardingService
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, portForwardingService := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(portForwardingService))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[port_forwarding_service.Name]*port_forwarding_service.PortForwardingService{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *port_forwarding_service.PortForwardingService
		switch item := change.ChangeType.(type) {
		case *port_forwarding_service.PortForwardingServiceChange_Added_:
			currentItem = item.Added.GetPortForwardingService()
		case *port_forwarding_service.PortForwardingServiceChange_Current_:
			currentItem = item.Current.GetPortForwardingService()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*port_forwarding_service.PortForwardingServiceChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *port_forwarding_service.PortForwardingServiceChange_Added_:
			newItem := item.Added.GetPortForwardingService()
		...
		case *port_forwarding_service.PortForwardingServiceChange_Modified_:
			updatedItem := item.Modified.GetPortForwardingService()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*port_forwarding_service.PortForwardingServiceChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetPortForwardingServiceName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(port_forwarding_service.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*port_forwarding_service.PortForwardingService_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*port_forwarding_service.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(port_forwarding_service_client.NewPortForwardingServiceServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(port_forwarding_service.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*port_forwarding_service.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*port_forwarding_service.ParentReference)
		}
		return params
	})
}
